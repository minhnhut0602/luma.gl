...
test('Core#Attribute update/read props', t => {
  const {gl, gl2} = fixture;
  ...
  buffer = new Buffer(gl, {bytes: 1024, type: GL.FLOAT, divisor: 1});
  attribute.update({buffer});
  t.is(attribute.accessor.divisor, 1, 'divisor prop is updated using buffer prop');
...
test('Core#Attribute update/read props (DEPRECATED)', t => {
  ...
  // gpu aggregation use case
  const buffer = new Buffer(gl, {bytes: 1024, type: GL.FLOAT, instanced: 1});
  attribute.update({buffer});
